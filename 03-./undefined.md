...
하지만 `해시 테이블(Hash Table)`또한 장점이 있으니 단점이 있습니다. 그것은 바로 해시 함수의 오류로 인한 해시 인덱스 값의 충돌이 발생할 수 있다는 점입니다.
...
처음 `맵(Map)`을 할당하게 되면 `nil 값`을 가지고 있으며, 이를 `Nil Map`이라고 부른다. `Nil Map`은 **초기화를 통해 원하는 형태의 데이터를 할당**할 수 있습니다.
